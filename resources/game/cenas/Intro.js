...
export class Intro extends BaseCena {
    ...
    create() {
        this.currentBgIndex = 0;
        this.background = this.add.image(0, 0, this.introAssets[this.currentBgIndex]).setOrigin(0, 0);
    ...
    goToNextBackground() {
        if (this.currentBgIndex >= this.introAssets.length - 1) {
            return;
        }
    ...
    updateBackgroundAndButtons() {
        ...
        const isLast = this.currentBgIndex === this.introAssets.length - 1;

        this.background.setTexture(this.introAssets[this.currentBgIndex]);
